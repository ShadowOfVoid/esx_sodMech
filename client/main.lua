...
Citizen.CreateThread(function()
  while true do
    ...
    if CurrentAction ~= nil then
      ...
      if IsControlJustReleased(0, 38) then  --buton --38e --182l
...
AddEventHandler('esx_SodaMachine:Random', function(prop_name)
	...
	if lottery <= 6 then
		if not IsAnimated then
			...
			Citizen.CreateThread(function()
				...
				exports['mythic_notify']:DoHudText('inform', 'Soda geliyor.. Sabret !')
				...
				Citizen.Wait(700)
				exports['mythic_notify']:DoHudText('inform', 'Az önce 1$ ödeyerek bir şişe soda satın aldın!')
	...
	if lottery == 7 then
			Citizen.CreateThread(function()
			exports['mythic_notify']:DoHudText('inform', 'Bu makine geçici olarak çalışmıyor.')
			end)
		end
	if lottery == 8 then
			Citizen.CreateThread(function()
			exports['mythic_notify']:DoHudText('inform', 'Bu makine bu ürünün dışında.')
			end)
		end
	if lottery == 9 then
			Citizen.CreateThread(function()
			...
			TriggerEvent('InteractSound_CL:PlayOnOne', 'SodaMachine', 1.0)
			Citizen.Wait(5000)
			exports['mythic_notify']:DoHudText('inform', 'Makine paranı yuttu ve sodanı vermedi !')
			FreezeEntityPosition(ped, false)
			end)
		end
end)